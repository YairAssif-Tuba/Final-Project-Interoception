"""
Enhanced Piezo vs Non-Piezo Network Comparison Script - CLEAN INTEGRATION
========================================================================

This script is IDENTICAL to piezo_comparison.py with these ADDITIONS:
1. Piezo connectivity parameter statistics (initial vs final + correlation)
2. Training time and convergence analysis (time to reach 90%/95% thresholds)
3. All original functionality preserved

CHANGES FROM ORIGINAL:
+ Enhanced connectivity tracking for piezo models
+ Convergence timing analysis
+ Additional statistics saved
- Non-piezo training uncommented (fixed)

Usage:
    python enhanced_piezo_comparison.py [--full-training] [--time-delay] [--output-dir OUTPUT_DIR]
"""
...
class DatasetLoader:
    ...
    def get_next_params(self, batch_size):
        ...
        return params
class EnhancedPiezoComparisonAnalyzer:
    """Enhanced analyzer - IDENTICAL to original with connectivity & convergence tracking added."""

    def __init__(self, output_dir="enhanced_piezo_comparison_results", use_time_delay=False, num_runs=10):
        self.output_dir = output_dir
        self.use_time_delay = use_time_delay
        self.num_runs = num_runs
        tools.mkdir_p(self.output_dir)

        # IDENTICAL enhanced intervals from original
        ...
        print(f"🔬 Enhanced Piezo Comparison Analyzer")
    ...
    def create_enhanced_hp(self, use_piezo=False, use_insula=False):
        """MODIFIED: Use interval_comparison instead of interval_production, support insula"""
        hp = default.get_default_hp('interval_comparison', use_piezo=use_piezo, use_insula=use_insula)
    ...
    def modify_task_generation(self):
        """IDENTICAL to original piezo_comparison.py"""
        import task

        # Store original function
        original_interval_comparison = task._interval_comparison
        dataset_loader = self.dataset_loader

        def wrapped_original(*args, **kwargs):
            print("DEBUG: ORIGINAL _interval_comparison called!")
            return original_interval_comparison(*args, **kwargs)

        def enhanced_interval_comparison(config, mode, **kwargs):
            """Enhanced interval comparison with larger intervals."""
            print("DEBUG: enhanced_interval_comparison called!")
            # Call original function first
            trial = original_interval_comparison(config, mode, **kwargs)
            dt = config['dt']
            batch_size = trial.batch_size
            # Replace the part that generates intervals:
            if dataset_loader is not None:
                print(f"DEBUG: Taking dataset_loader path")
                params = dataset_loader.get_next_params(batch_size)

                trial.prod_interval1 = np.array([p['prod_interval1'] / dt for p in params]).astype(int)
                trial.prod_interval2 = np.array([p['prod_interval2'] / dt for p in params]).astype(int)
                trial.dly_interval = np.array([p['dly_interval'] / dt for p in params]).astype(int)

                if self.dataset_loader.total_samples_used <= 5:
                    print(f"VERIFICATION: interval1={params[0]['prod_interval1']:.1f}ms")
            ...
            # If we have enhanced intervals config, override the intervals
                ...
                if mode in ['random', 'random_validate']:
                    ...
                    # Override with enhanced intervals
                    trial.prod_interval1 = (rng.uniform(min_int, max_int, batch_size) / dt).astype(int)
                    trial.prod_interval2 = (rng.uniform(min_int, max_int, batch_size) / dt).astype(int)
                    trial.dly_interval = np.array([delay_int] * batch_size)
                    trial.dly_interval = (trial.dly_interval / dt).astype(int)
        ...
        # Patch the function
        task._interval_comparison = enhanced_interval_comparison
        task.rule_mapping['interval_comparison'] = enhanced_interval_comparison
        # NEW: Verify the patching worked
        print(f"DEBUG: After patching, rule_mapping function name: {task.rule_mapping['interval_comparison'].__name__}")
        print(f"DEBUG: After patching, rule_mapping function id: {id(task.rule_mapping['interval_comparison'])}")
        print(f"DEBUG: Enhanced function name: {enhanced_interval_comparison.__name__}")
        print(f"DEBUG: Enhanced function id: {id(enhanced_interval_comparison)}")
        print(f"DEBUG: Task patching complete. Function is now: {task._interval_comparison}")
        print(f"DEBUG: dataset_loader exists: {self.dataset_loader is not None}")

        print("✅ Task generation modified for enhanced intervals")
    ...
    def _regenerate_trial_with_intervals(self, trial, config):
        ...
        prod_interval1 = trial.prod_interval1
        prod_interval2 = trial.prod_interval2
        ...
        new_trial.add('input', 1, ons=stim2_on, offs=stim2_off, strengths=new_trial.expand(1.))

        # Output targets
        output_target1 = 1. * (prod_interval1 > prod_interval2)
        output_target2 = 1. * (prod_interval1 <= prod_interval2)

        new_trial.add('out', 0, ons=stim2_off, offs=xtdim, strengths=output_target1)
        new_trial.add('out', 1, ons=stim2_off, offs=xtdim, strengths=output_target2)
        new_trial.add('cost_mask', 0, ons=stim1_on, offs=xtdim, strengths=new_trial.expand(1.))
        new_trial.add('cost_mask', 1, ons=stim1_on, offs=xtdim, strengths=new_trial.expand(1.))
        ...
        new_trial.prod_interval1 = prod_interval1
        new_trial.prod_interval2 = prod_interval2
    ...
    # NEW ENHANCEMENT 1: Connectivity analysis methods
    ...
    def train_networks(self, max_samples=5e5):
        ...
        # Train non-piezo networks (UNCOMMENTED - was commented in original)
    ...
    def train_insula_networks(self, max_samples=5e5):
        ...
        print(f"\n🧠 Enhanced Insula Network Training (Interval Comparison)")
        print(f"   Max samples per run: {max_samples:,.0f}")
        print(f"   Runs per network type: {self.num_runs}")
    ...
    def _train_with_enhanced_tracking(self, model_dir, use_piezo, max_samples, display_step, run_idx, use_insula=False):
        """IDENTICAL to original _train_with_retry but with enhanced connectivity tracking."""
        import shutil
        ...
        while True:
            ...
            try:
                ...
                # Create trainer (IDENTICAL to original)
                ...
                if use_piezo:
                    if hasattr(trainer.model, 'piezo_connectivity') and trainer.model.piezo_connectivity is not None:
                        ...
                        print(f"      ⚠️ No piezo_connectivity found in model")
                elif use_insula:
                    ...
                    if hasattr(trainer.model, 'insula') and trainer.model.insula is not None:
                        ...
                        print(f"      ⚠️ No insula found in model")
    ...
    # NEW ENHANCEMENT 2: Convergence timing analysis
    ...
    def analyze_eigenvalues(self, training_results):
        """IDENTICAL to original piezo_comparison.py analyze_eigenvalues method."""
        ...
        for network_type, network_results in training_results.items():
            ...
            for run_result in network_results['runs']:
                if run_result['status'] != 'OK' or run_result['trainer'] is None:
                    print(f"   ⚠️ Skipping run {run_result['run_idx']} - training failed")
                    continue
                ...
                if hp is None:
                    print(f"   ⚠️ Skipping run {run_result['run_idx']} - no hyperparameters")
                    continue

                try:
                    # Create and load model
                    model = network.RNN(hp, is_cuda=True, rule_name='interval_comparison')
                    ...
                    if not model.load(model_dir):
                        ...
                        if os.path.exists(final_result_dir) and not model.load(final_result_dir):
                            print(f"   ⚠️ Could not load run {run_result['run_idx']} model")
                            continue
        ...
        # Create visualization with individual runs (IDENTICAL to original)
        if eigenvalue_results:
            ...
            print(f"\n⚠️ No successful networks to analyze eigenvalues")

        return eigenvalue_results

    def _plot_individual_runs_eigenvalues(self, eigenvalue_results):
        """IDENTICAL to original piezo_comparison.py plotting method."""
        if not eigenvalue_results:
            print("⚠️ No eigenvalue results to plot")
            return
        ...
        for network_type, data in eigenvalue_results.items():
            if 'eigenvalues_per_run' not in data:
                print(f"⚠️ Skipping {network_type} - no eigenvalue data")
                continue
    ...
    def _print_detailed_statistics(self, eigenvalue_results):
        """IDENTICAL to original piezo_comparison.py detailed statistics method."""
    ...
    def simulate_piezo_response(self):
        """IDENTICAL to original piezo_comparison.py simulate_piezo_response method."""
        ...
        print(f"   ⏱️ This is what happens during ONE timing task!")
    ...
    def _plot_piezo_response(self, slice_times, slice_means, piezo_responses,
        ...
        # Show task phases
        interval1_end = max_interval_sec
        delay_end = interval1_end + 1.0  # 1 second delay
        interval2_end = delay_end + max_interval_sec

        ax2.axvspan(0, interval1_end, alpha=0.2, color='green', label='Interval 1')
        ax2.axvspan(interval1_end, delay_end, alpha=0.2, color='yellow', label='Delay')
        ax2.axvspan(delay_end, interval2_end, alpha=0.2, color='orange', label='Interval 2')

        ax2.set_ylabel('Mean Pressure')
        ax2.set_title('Cardiac Pressure During Task Phases')
        ax2.grid(True, alpha=0.3)
        ax2.legend()
        ...
        # Same task phases
        ax3.axvspan(0, interval1_end, alpha=0.2, color='green', label='Interval 1')
        ax3.axvspan(interval1_end, delay_end, alpha=0.2, color='yellow', label='Delay')
        ax3.axvspan(delay_end, interval2_end, alpha=0.2, color='orange', label='Interval 2')

        ax3.set_ylabel('Piezo Response')
        ax3.set_title(f'Piezo Response During Task (Time Delay: {self.use_time_delay})')
        ax3.grid(True, alpha=0.3)
        ax3.legend()
    ...
    def _plot_individual_model_performance(self, training_results):
        ...
        for network_type, network_results in training_results.items():
            ...
            for run_result in network_results['runs']:
                ...
                # Plot 3: Choice error over time (log scale)
                ax3.semilogy(times, choice_errors, 'orange', marker='^', linewidth=2, markersize=6)
                ...
                plt.suptitle(f'{network_type.title()} Model Run {run_idx} Performance Analysis',
    ...
    def _save_multiple_runs_summary(self, results):
        """IDENTICAL to original piezo_comparison.py summary saving method."""
        summary = {
    ...
    def run_insula_analysis(self, max_samples=5e5, load_existing=False):
        ...
        print(f"\n🧠 ENHANCED INSULA NETWORK ANALYSIS (COMPARISON TASK)")
        print("=" * 80)
        print(f"📊 Analysis Parameters:")
        print(f"   🎯 Task: Interval Comparison")
        print(f"   🔍 Enhanced Intervals: {self.enhanced_intervals['min_interval']}-{self.enhanced_intervals['max_interval']}ms")
        print(f"   🔢 Max Samples: {max_samples:,.0f}")
        print(f"   🔄 Runs per network: {self.num_runs}")
        print(f"   ⏱️ Time Delay: {'✅ ENABLED' if self.use_time_delay else '❌ DISABLED'}")
        print(f"   📁 Load existing: {'✅ ENABLED' if load_existing else '❌ DISABLED'}")
        print(f"   🔍 Output: {self.output_dir}")
        print("🆕 ENHANCEMENTS:")
        print("   🧠 Insula gate tracking (initial vs final values)")
        print("   ⏱️ Convergence timing analysis (time to reach 90%/95% thresholds)")
        ...
        if load_existing:
            ...
            print("   ⚠️ This enhanced version focuses on training new networks")
            print("   ⚠️ For loading existing, use original piezo_comparison.py")
            return None
        else:
    ...
    def run_full_comparison(self, max_samples=5e5, load_existing=False):
        ...
        print(f"\n🔬 ENHANCED PIEZO VS NON-PIEZO COMPREHENSIVE COMPARISON")
        print("=" * 80)
        print(f"📊 Analysis Parameters:")
        print(f"   🎯 Task: Interval Comparison")
        print(f"   🔍 Enhanced Intervals: {self.enhanced_intervals['min_interval']}-{self.enhanced_intervals['max_interval']}ms")
        ...
        # Step 4: Analyze eigenvalues and spectral radii
        print(f"\n🔍 Step 4: Individual Runs Eigenvalue & Spectral Radius Analysis")
        eigenvalue_results = self.analyze_eigenvalues(training_results)

        # Step 5: Generate final enhanced report
        print(f"\n🔍 Step 5: Enhanced Final Report")
    ...
    def _generate_enhanced_report(self, training_results, eigenvalue_results, piezo_data,
        ...
        report = []
        report.append("ENHANCED PIEZO VS NON-PIEZO NETWORK COMPARISON")
        report.append("=" * 60)
        ...
        if connectivity_stats:
            ...
            for conn_data in connectivity_stats:
                ...
                if conn_data.get('initial') and conn_data.get('final'):
                    ...
                    initial_correlations.append(initial['activated_mean'])
                    final_correlations.append(final['activated_mean'])

            if raw_correlations:
                report.append(f"Connectivity Parameter Evolution:")
                report.append(
                    f"  Raw weight correlation: {np.mean(raw_correlations):.3f} ± {np.std(raw_correlations):.3f}")
                report.append(
                    f"  Activated weight correlation: {np.mean(activated_correlations):.3f} ± {np.std(activated_correlations):.3f}")
                report.append(f"  Max weight change: {np.mean(weight_changes):.3f} ± {np.std(weight_changes):.3f}")

            if initial_correlations and final_correlations:
                report.append(
                    f"  Initial connectivity mean: {np.mean(initial_correlations):.3f} ± {np.std(initial_correlations):.3f}")
                report.append(
                    f"  Final connectivity mean: {np.mean(final_correlations):.3f} ± {np.std(final_correlations):.3f}")

                # Statistical test for change
                from scipy import stats
                t_stat, p_value = stats.ttest_rel(final_correlations, initial_correlations)
            ...
            if conv_data['successful_runs'] > 0:
                ...
                for threshold_name, stats in conv_data['convergence_stats'].items():
                    ...
                    if achieved > 0:
                        ...
                        avg_steps = np.mean(stats['steps'])
                        report.append(
                            f"    {threshold_name}: {success_rate:.1f}% achieved, avg {avg_time:.1f}s, {avg_steps:.0f} steps")
                    else:
        ...
        for network_type, stats in accuracy_stats.items():
            ...
            report.append(
                f"  Mean success probability: {stats['mean_success_prob']:.3f} ± {stats['std_success_prob']:.3f}")
        ...
        if eigenvalue_results:
            for network_type, data in eigenvalue_results.items():
                ...
                report.append(
                    f"  Spectral radius: {data['mean_spectral_radius']:.4f} ± {data['std_spectral_radius']:.4f}")
                report.append(
                    f"  Spectral radius range: [{data['min_spectral_radius']:.4f}, {data['max_spectral_radius']:.4f}]")

                # Individual run statistics summary
        ...
        if 'no_piezo' in training_results and 'piezo' in training_results:
            no_piezo_time = training_results['no_piezo']['total_training_time'] / max(
                training_results['no_piezo']['successful_runs'], 1)
            piezo_time = training_results['piezo']['total_training_time'] / max(
                training_results['piezo']['successful_runs'], 1)
        ...
        if len(convergence_results) >= 2:
            if 'no_piezo' in convergence_results and 'piezo' in convergence_results:
                no_piezo_conv = convergence_results['no_piezo']
                piezo_conv = convergence_results['piezo']

                report.append(f"Convergence Efficiency:")

                # Compare 90% accuracy achievement
                if 'accuracy_90' in no_piezo_conv['convergence_stats'] and 'accuracy_90' in piezo_conv[
                    'convergence_stats']:
                    no_piezo_90 = no_piezo_conv['convergence_stats']['accuracy_90']['achieved_count']
                    piezo_90 = piezo_conv['convergence_stats']['accuracy_90']['achieved_count']

                    no_piezo_rate = no_piezo_90 / no_piezo_conv['successful_runs'] if no_piezo_conv[
                                                                                          'successful_runs'] > 0 else 0
                    piezo_rate = piezo_90 / piezo_conv['successful_runs'] if piezo_conv['successful_runs'] > 0 else 0

                    report.append(f"  90% accuracy achievement:")
                    report.append(f"    No-Piezo: {no_piezo_rate * 100:.1f}%")
                    report.append(f"    Piezo: {piezo_rate * 100:.1f}%")

                    if piezo_rate > no_piezo_rate + 0.1:
                        report.append(f"    Piezo models converge more reliably")
                    elif no_piezo_rate > piezo_rate + 0.1:
                        report.append(f"    No-Piezo models converge more reliably")
                    else:
                        report.append(f"    Similar convergence reliability")

        report.append("")

        # Enhanced Recommendations
        report.append("ENHANCED RECOMMENDATIONS")
        report.append("-" * 25)

        # Overall assessment
        overall_success = total_successful / (self.num_runs * 2)
        if overall_success >= 0.8:
            report.append("Enhanced training pipeline is robust")
        elif overall_success >= 0.6:
            report.append("Enhanced training pipeline has moderate reliability")
        else:
            report.append("Enhanced training pipeline needs improvement")
            # Connectivity-specific recommendations
        if connectivity_stats:
            avg_raw_corr = np.mean(raw_correlations) if raw_correlations else 0
            if avg_raw_corr > 0.8:
                report.append("Connectivity parameters show high stability during training")
                report.append("   Consider reducing connectivity learning rate for efficiency")
            elif avg_raw_corr < 0.3:
                report.append("Connectivity parameters show high plasticity during training")
                report.append("   This suggests active adaptation to cardiac input")
            else:
                report.append("Connectivity parameters show moderate adaptation during training")

            # Accuracy-specific recommendations
        if accuracy_stats:
            for network_type, stats in accuracy_stats.items():
                if stats['mean_accuracy'] < 0.85:
                    report.append(f"{network_type.upper()} models show suboptimal accuracy")
                    report.append(f"   Consider adjusting learning rate or regularization")
                elif stats['std_accuracy'] > 0.1:
                    report.append(f"{network_type.upper()} models show high accuracy variance")
                    report.append(f"   Consider more consistent initialization or training parameters")

        report.append("")
        report.append("ENHANCED CHANGES IMPLEMENTED:")
        report.append("Connectivity parameter tracking (initial vs final + correlation)")
        report.append("Convergence timing analysis (time to reach 90%/95% thresholds)")
        report.append("Individual model performance graphs")
        report.append("Mean accuracy analysis with detailed statistics")
        report.append("Enhanced statistical comparisons")
        report.append("Comprehensive checkpoint analysis (numbered + finalResult)")
        report.append("Non-piezo training uncommented (fixed from original)")
    ...
    def _generate_enhanced_insula_report(self, training_results, eigenvalue_results, piezo_data,
        ...
        report = []
        report.append("ENHANCED INSULA NETWORK ANALYSIS (COMPARISON TASK)")
        report.append("=" * 60)
        report.append(f"Generated: {time.strftime('%Y-%m-%d %H:%M:%S')}")
        report.append(f"Training runs per network: {self.num_runs}")
        report.append(f"Time Delay: {'ENABLED' if self.use_time_delay else 'DISABLED'}")
        report.append(f"Max samples per run: {5e5:,.0f}")
        report.append("")
        report.append("TASK: INTERVAL COMPARISON")
        report.append("Networks learn to discriminate between two time intervals")
        report.append("See two intervals -> output which one is longer")
        report.append("")
        ...
        if insula_gate_stats:
            ...
            initial_gate = 0.2  # Default from hp
        ...
        # Mean Accuracy Analysis
        report.append("MEAN ACCURACY ANALYSIS (COMPARISON TASK)")
        report.append("-" * 38)
...
def main():
    """Main function to run the enhanced comparison analysis."""
    parser = argparse.ArgumentParser(description='Enhanced Piezo vs Non-Piezo Network Comparison')
    ...
    parser.add_argument('--output-dir', default='enhanced_piezo_comparison_results',
    ...
    if args.load_existing:
        print("⚠️ Loading existing models is not implemented in the enhanced version.")
    ...
    analyzer = EnhancedPiezoComparisonAnalyzer(
