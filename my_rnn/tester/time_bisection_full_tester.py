"""
Enhanced Time Bisection Full Tester - UNIFIED VERSION
====================================================

Tests all three types of trained time bisection models with comprehensive analysis:
...
class EnhancedTimeBisectionFullTester:
    """Unified tester for all three types of time bisection models"""
    ...
    def __init__(self, base_results_dir="time_bisection_full_results"):
        ...
        print(f"Enhanced Time Bisection Full Tester initialized")
    ...
    def _load_best_piezo_library(self):
        ...
        try:
            ...
            print("Using default piezo library: hr60_hrv0cal")
            return "hr60_hrv0cal"
    ...
    def _load_best_insula_library(self):
        ...
        try:
            ...
            print("Using default insula library: hr60_hrv0cal")
            return "hr60_hrv0cal"
    ...
    def _create_ood_conditions(self):
        ...
        for duration in [2200, 2400, 2600, 2800]:  # Ambiguous durations for 2000-3000 framework
            condition = {
                'duration': float(duration),
                'short_standard': 2000,
                'long_standard': 3000,
    ...
    def test_single_model(self, model_dir, rule_name, test_conditions):
        ...
        # Initialize runner with proper hyperparameters (matching time bisection tester logic)
            ...
            while not successful and attempts < max_attempts:
                ...
                try:
                    ...
                    if use_piezo or use_insula:
                        ...
                        task_duration_ms = condition['duration'] + condition.get('response_duration', 500) + 500  # Add buffer

                        # Use create_real_cardiac_data_for_task directly (like time bisection tester)
                        cardiac_data = create_real_cardiac_data_for_task(
                            ...
                            cardiac_library=self.best_insula_library if use_insula else self.best_piezo_library
                        )
                        ...
                        if attempts == 1:
                            ...
                            print(f"    Loaded real cardiac data from library: {library_used}")
                    else:
                        condition_with_cardiac = condition.copy()
                    ...
                    # Calculate performance using time bisection performance method
                    ...
                    has_nan = has_nan_outputs or has_nan_cost or performance_has_nan
                    ...
                    if has_nan:
                        ...
                        if attempts < max_attempts:
                            ...
                            results['nan_conditions'].append(
                                f"Condition {i + 1}: duration={condition['duration']:.0f}ms, standards={condition['short_standard']:.0f}/{condition['long_standard']:.0f}ms")
                    else:
                        successful = True
                        if attempts > 1:
                    ...
                    if attempts >= max_attempts:
                        ...
                        results['nan_conditions'].append(
                            f"Condition {i + 1}: duration={condition['duration']:.0f}ms, standards={condition['short_standard']:.0f}/{condition['long_standard']:.0f}ms (Exception)")
                        break
    ...
    def _calculate_time_bisection_performance(self, trial, outputs, condition):
        """Calculate time bisection performance using the robust logic from time bisection tester"""
        y = trial.y  # [T, B, O] - targets from task generation
        T, B, O = outputs.shape

        # Find response timing (from time bisection tester logic)
    ...
    def _summarize_performance(self, test_results):
        ...
        # Time bisection task - focus on accuracy
    ...
    def plot_three_way_performance_comparison(self, all_results):
        ...
        fig.suptitle('3-Way Performance Comparison: Standard vs Piezo vs Insula Models (Time Bisection)', fontsize=16)
    ...
    def plot_duration_vs_accuracy_three_way(self, all_results, save_path=None):
        """Create scatter plot of duration vs accuracy for all three model types"""

        # Filter for time bisection task results only
        bisection_results = [r for r in all_results if r['rule_name'] == 'time_bisection']

        if not bisection_results:
            print("No time bisection results found for scatter plot")
            return

        # Collect data for scatter plot organized by reference framework
        framework_data = {}  # (short_std, long_std): {'standard': {duration: [accuracies]}, 'piezo': {duration: [accuracies]}, 'insula': {duration: [accuracies]}}

        for result in bisection_results:
            ...
            for test_result in result['test_results']:
                ...
                if framework_key not in framework_data:
                    framework_data[framework_key] = {'standard': {}, 'piezo': {}, 'insula': {}}
        ...
        fig.suptitle('Duration vs Accuracy by Reference Framework - 3-Way Comparison\n(Time Bisection Task)', fontsize=16,
                     fontweight='bold')
        ...
        for idx, (framework_key, data) in enumerate(framework_data.items()):
            ...
            is_id_framework = (short_std == 1000 and long_std == 2000)  # Assuming this is the training framework
            ...
            if standard_data:
                ...
                for duration in sorted(standard_data.keys()):
                    accuracies = standard_data[duration]
                    standard_durations.append(duration)
                    standard_means.append(np.mean(accuracies))
                    standard_stds.append(np.std(accuracies))

                ax.errorbar(standard_durations, standard_means, yerr=standard_stds,
                            fmt='o', color='blue', markersize=8, capsize=5,
                            label=f'Standard Models (n={len(standard_data)} durations)',
            ...
            # Plot insula models
            insula_data = data['insula']
            if insula_data:
                insula_durations = []
                insula_means = []
                insula_stds = []

                for duration in sorted(insula_data.keys()):
                    accuracies = insula_data[duration]
                    insula_durations.append(duration)
                    insula_means.append(np.mean(accuracies))
                    insula_stds.append(np.std(accuracies))

                ax.errorbar(insula_durations, insula_means, yerr=insula_stds,
                            fmt='^', color='green', markersize=8, capsize=5,
                            label=f'Insula Models (n={len(insula_data)} durations)',
                            alpha=0.8, linewidth=2)
            ...
            # Mark bisection point and standards
        ...
        if save_path is None:
            save_path = os.path.join(self.base_results_dir, "duration_vs_accuracy_three_way_scatter.png")

        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.close()

        print(f"Saved 3-way duration vs accuracy scatter plot: {save_path}")

        # Print summary statistics
        print(f"\n3-Way Scatter Plot Summary:")
        for framework_key, data in framework_data.items():
            short_std, long_std = framework_key
            print(f"  Framework {short_std}-{long_std}ms:")
            print(f"    Standard models: {len(data['standard'])} different durations")
            print(f"    Piezo models: {len(data['piezo'])} different durations")
            print(f"    Insula models: {len(data['insula'])} different durations")

            if data['standard']:
                total_standard_points = sum(len(accuracies) for accuracies in data['standard'].values())
                print(f"    Total standard data points: {total_standard_points}")

            if data['piezo']:
                total_piezo_points = sum(len(accuracies) for accuracies in data['piezo'].values())
                print(f"    Total piezo data points: {total_piezo_points}")

            if data['insula']:
                total_insula_points = sum(len(accuracies) for accuracies in data['insula'].values())
                print(f"    Total insula data points: {total_insula_points}")

    def discover_all_bisection_models(self):
        """Discover all trained time bisection models (standard, piezo, and insula)"""
        model_dirs = []

        # Define possible base directories for all model types
        possible_base_dirs = [
            'enhanced_piezo_time_bisection_results',  # All three model types
            'bisection_results',  # Mixed
            'time_bisection_results',  # Mixed
            'model/time_bisection',  # Mixed
        ]
    ...
    def _generate_enhanced_summary_report(self, all_results, test_conditions):
        ...
        report_path = os.path.join(self.base_results_dir, "enhanced_full_bisection_test_report.txt")
        ...
        with open(report_path, 'w') as f:
            f.write("ENHANCED TIME BISECTION FULL DATASET TEST REPORT\n")
            f.write("=" * 70 + "\n\n")
    ...
    def run_enhanced_full_dataset_test(self, num_conditions=15, include_ood=True):
        ...
        print("Starting Enhanced Time Bisection Full Dataset Test")
        print("=" * 80)

        # Discover all models
        model_directories = self.discover_all_bisection_models()
        ...
        if all_results:
            ...
            # 3-way duration vs accuracy scatter plot
            self.plot_duration_vs_accuracy_three_way(all_results)

            # Generate statistical analysis
            analyzer = ThreeWayStatisticalAnalyzer()
            standard_data, piezo_data, insula_data = analyzer.extract_performance_data(all_results, 'time_bisection')
            statistical_results = analyzer.perform_three_way_statistical_tests(standard_data, piezo_data, insula_data, 'time_bisection')
            
            # Generate and save statistical report
            stat_report = analyzer.generate_three_way_statistical_report(statistical_results, 'time_bisection')
        ...
        print(f"  - 3-way duration vs accuracy: duration_vs_accuracy_three_way_scatter.png")
        print(f"Generated reports:")
        print(f"  - 3-way statistical analysis: three_way_statistical_analysis.txt")
        print(f"  - Enhanced summary report: enhanced_full_bisection_test_report.txt")

        return all_results


def main():
    """Run enhanced time bisection full dataset test with 3-way statistical analysis"""
    tester = EnhancedTimeBisectionFullTester()
